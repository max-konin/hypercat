...
resource 'Mappings' do
  ...
  get '/api/v1/mappings.json' do
    let!(:mapping_1){create :mapping, hypernet: hypernet}
    let!(:mapping_2){create :mapping, hypernet: hypernet}
  ...
  get '/api/v1/mappings/:id.json' do
    parameter :id, "Mapping's ID", required: true
    parameter :hypernet_id, "Hypernet's ID", required: true

    let!(:mapping){create :mapping, hypernet: hypernet}
    let(:id){mapping.id}
    ...
    example 'Get a mappings by id' do
      ...
      expect(status).to eq 200
      expect(response_body).to eq ActiveModel::SerializableResource.new(mapping).to_json
    end
  end

  post '/api/v1/mappings.json' do
    parameter :branch_id, "Edge of a primary network", scope: :mapping, required: true
    parameter :edge_ids, "Edges of a secondary network", scope: :mapping
    parameter :hypernet_id, "Hypernet's ID", required: true
    ...
    example 'Creating a mapping' do
      ...
      expect{do_request}.to change{Mapping.count}.by(1)

      expect(status).to eq 201
    end
  end

  delete '/api/v1/mappings/:id.json' do
    parameter :id, "Mapping's ID", required: true
    parameter :hypernet_id, "Hypernet's ID", required: true
  ...
  put '/api/v1/mappings/:id.json' do
    parameter :branch_id, "Edge of a primary network", scope: :mapping, required: true
    parameter :edge_ids, "Edges of a secondary network", scope: :mapping
    parameter :hypernet_id, "Hypernet's ID", required: true
